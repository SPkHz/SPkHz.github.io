...
function [IL_num, IL_den, R2_num, R2_den] = ...
    EE456_IL_Function_f0_gpu(fL, fH, k0, kT, N_Poly, NSi)
...
for kk = 1 : N_BPF
    if kk > length(sz_LPF)
        warning(...
            'Not enough roots in sz_LPF. Truncating filter at kk = %d.',...
            kk);
        break;
    end
...
IL_num = poly(sz_BPF);
IL_num = abs(X_Scale/polyval(IL_num, 1j)) * IL_num;
I_max = length(IL_num);
IL_num(2 : 2 : I_max) = 0;
...
IL_den(I_Slope) = X_Sign(I_Slope) * (f0/fH)^(2*NSi);
IL_den(2 : 2 : I_max) = 0;
