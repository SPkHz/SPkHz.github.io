% EE_457_Design_Project_03_Placzek_Matlab_read_csv.m
...
%   - All traces get -3 dB markers at crossings.
%   - A  -3 dB bandwidth is computed and printed for each trace.
%   - A -10 dB bandwidth is computed and printed for the V4 trace.
...
PlotSpec.markerLevel_dB = -3;         % Level for bandwidth markers (dB)
...
for k = 1:nFiles
    ...
    if ~isnan(bw.BWabs)
        ...
        fprintf('f_low = %.4f GHz, f_high = %.4f GHz, ', ...
                bw.fLow, bw.fHigh);
        fprintf('BW = %.4f GHz (%.4f%%)\n', bw.BWabs, bw.BWpercent);
    else
        fprintf('%s: could not find two crossings at %.1f dB.\n', ...
                name, PlotSpec.markerLevel_dB);
    end

    % ===== -10 dB bandwidth only for V4 =====
    if contains(name, 'V4')   % or (k == 4) if V4 is always 4th file
        bw10 = computeBandwidthFromLevel(f, s11, -10);

        if ~isnan(bw10.BWabs)
            fprintf('>> %s (-10 dB): f0 ≈ %.4f GHz, ', name, bw10.f0);
            fprintf('f_low = %.4f GHz, f_high = %.4f GHz, ', ...
                    bw10.fLow, bw10.fHigh);
            fprintf('BW = %.4f GHz (%.4f%%)\n', ...
                    bw10.BWabs, bw10.BWpercent);
        else
            fprintf('>> %s: could not find two crossings at -10 dB.\n', name);
        end
    end
end
...
% Find frequency and |S11| columns by name
...
ylabel('$|S_{11}|$ (dB)', 'Interpreter', 'latex', ...
    'FontSize', PlotSpec.fontSizeLabel);
title(PlotSpec.titleString, 'Interpreter', 'latex', ... % New title
    'FontSize', PlotSpec.fontSizeLabel);
